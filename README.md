...
reusable workflow invokes the root action with `uses: $/`. Both resolve from
the caller commit. The candidate tag is passed as an input only to verify the
immutable release and asset.
